...
// import { restaurantApi, Restaurant, getResolvedBaseUrl } from '@/lib/api';
...
//   const qrSlug = (restaurant as any)?.qrSlug || restaurant?._id || '';
//   const qrRedirectEnabled = !!(restaurant as any)?.whatsapp?.isEnabled && !!(restaurant as any)?.whatsapp?.qrRedirectEnabled;

//   const menuUrl = restaurant 
//     ? `${window.location.origin}/menu/${qrSlug}`
//     : '';

//   // When WhatsApp redirect is enabled, the QR encodes the backend URL directly.
//   // The backend immediately 302s to WhatsApp before React ever loads — zero flash.
//   // When disabled, the QR encodes the normal frontend menu URL.
//   const backendScanUrl = qrSlug
//     ? `${getResolvedBaseUrl()}/api/public-menu/${qrSlug}`
//     : '';
//   const qrScanUrl = qrRedirectEnabled ? backendScanUrl : menuUrl;
...
//   //   const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${encodeURIComponent(qrScanUrl)}`;
...
//   // const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(qrScanUrl)}&color=f59e0b&bgcolor=0f0f11`;
// const qrImageUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(qrScanUrl)}&color=f59e0b&bgcolor=0f0f11`;
//   return (
//     <div className="w-full max-w-2xl mx-auto space-y-6 sm:space-y-8">
//       {/* Header */}
...
//           {qrRedirectEnabled
//             ? 'QR goes directly to WhatsApp — no menu screen.'
//             : 'QR opens your menu directly.'}
//         </p>
//       </div>
...
export default function QRCodePage() {
  ...
  const menuUrl = restaurant 
    ? `${window.location.origin}/menu/${qrSlug}`
    : '';
  // const qrScanUrl = qrSlug
  //   ? `${menuUrl}${qrRedirectEnabled ? '?source=qr' : ''}`
  //   : '';
  const qrScanUrl = menuUrl;
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-6 sm:space-y-8">
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          {qrRedirectEnabled
            ? 'QR forwards to WhatsApp when scanned.'
            : 'QR opens your menu directly.'}
        </p>
      </div>
